...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let from_serde = quote! { ::rl_logger::Value::from_serde };
    let from_display = quote! { ::rl_logger::Value::from_display };
    let from_debug = quote! { ::rl_logger::Value::from_debug };
    let key_new = quote! { ::rl_logger::Key::new };
    ...
    let expanded = quote! {
        ...
        impl #impl_generics ::rl_logger::Schema for #name #ty_generics #where_clause {
            fn visit(&self, visitor: &mut dyn ::rl_logger::Visitor) {
                #(#visits)*
            }
        }
    };
