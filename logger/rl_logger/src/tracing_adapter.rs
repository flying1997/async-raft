...
/// A layer that translates tracing events into rl-logger events.
pub struct TracingToRloggerLayer;
...
impl<S> Layer<S> for TracingToRloggerLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-rl-logger] failed to translate event due to unknown level {:?}",
